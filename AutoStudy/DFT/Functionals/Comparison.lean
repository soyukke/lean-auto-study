...
-- GGA が満たす条件の数: 4 (+ LDA帰着)
-- 1. 交換非正性, 2. 相関非正性, 3. LDA交換帰着, 4. LDA相関帰着
-- LDA帰着を通じて均一密度極限も間接的に満たす
...
/-- GGA が満たす条件数（サイズ整合性を除く） -/
theorem gga_constraint_count : ggaSatisfied.length = 4 := rfl
...
/-- LDA の優位点: サイズ整合性は LDA の構造的利点 -/
theorem lda_has_size_consistency :
    sizeConsistent ∈ ldaSatisfied ∧ sizeConsistent ∉ ggaSatisfied := by
  constructor
  · simp [ldaSatisfied]
  · simp [ggaSatisfied]
